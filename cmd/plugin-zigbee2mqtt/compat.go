...
import (
	...
	app "github.com/slidebolt/plugin-zigbee2mqtt/app"
...
func resolveEntityName(discovery translate.DiscoveryPayload, entityType, objectID string) string {
	if discovery.Name != "" {
...
func (p *plugin) handleDiscoveryMessage(client mqtt.Client, msg mqtt.Message) {
	...
	var discovery translate.DiscoveryPayload
...
// matching entities and decode each according to its own type.
func (p *plugin) handleStateMessage(client mqtt.Client, msg mqtt.Message) {
	...
	deviceID, ok := p.deviceIDFromStateTopic(topic)
	if !ok {
		return
	}

	matches, err := p.findEntitiesByDevice(deviceID)
	...
	for _, m := range matches {
		...
		state, ok := translate.DecodeWithMeta(entity.Type, payload, m.Info.ValueField, m.Info.UnitOfMeasurement, m.Info.SensorDeviceClass)
		if !ok {
			continue
		}
		entity.State = state
		if err := p.store.Save(entity); err != nil {
			log.Printf("plugin-zigbee2mqtt: failed to update entity %s: %v", m.Key.Key(), err)
			continue
		}
		updated++
	}
	if updated > 0 {
		log.Printf("plugin-zigbee2mqtt: state update on %s (device %s) — updated %d entities", topic, deviceID, updated)
	}
}

func (p *plugin) deviceIDFromStateTopic(stateTopic string) (string, bool) {
	prefix := p.mqttCfg.BaseTopic + "/"
	if !strings.HasPrefix(stateTopic, prefix) {
		return "", false
	}
	deviceID := strings.TrimPrefix(stateTopic, prefix)
...
func (p *plugin) saveTopicInfo(key domain.EntityKey, info EntityTopicInfo) error {
	...
	return p.store.WriteFile(storage.Internal, key, data)
}

// entityMatch pairs an entity key with its internal topic info.
type entityMatch struct {
	Key  domain.EntityKey
	Info EntityTopicInfo
}

// findEntitiesByDevice returns all entities for a device, along with their
// topic info for field-accurate decoding.
func (p *plugin) findEntitiesByDevice(deviceID string) ([]entityMatch, error) {
	entries, err := p.store.Query(storage.Query{
		Where: []storage.Filter{
			{Field: "plugin", Op: storage.Eq, Value: pluginID},
			{Field: "deviceID", Op: storage.Eq, Value: deviceID},
		},
	})
	if err != nil {
		return nil, err
	...
	for _, entry := range entries {
		...
		matches = append(matches, entityMatch{Key: entityKey, Info: topicInfo})
	}
	return matches, nil
}
...
func (p *plugin) handleCommand(addr messenger.Address, cmd any) {
	...
	payload, err := translate.Encode(cmd, internal)
	if err != nil {
	...
	if p.mqtt != nil && p.mqtt.IsConnected() && topicInfo.CommandTopic != "" {
		// Convert to []byte explicitly to avoid "unknown payload type" error
		payloadBytes := []byte(string(payload))
		token := p.mqtt.Publish(topicInfo.CommandTopic, 0, false, payloadBytes)
		token.WaitTimeout(5 * time.Second)
		if token.Error() != nil {
			log.Printf("plugin-zigbee2mqtt: failed to publish to %s: %v", topicInfo.CommandTopic, token.Error())
		} else {
			log.Printf("plugin-zigbee2mqtt: published to %s: %s", topicInfo.CommandTopic, string(payload))
		}
	} else {
		// Log the command even if MQTT is not connected (useful for testing)
		log.Printf("plugin-zigbee2mqtt: %s command (MQTT not connected): %s", addr.Key(), string(payload))
	}
	...
	case domain.ClimateSetTemperature:
		...
		if climate, ok := entity.State.(domain.Climate); ok {
			climate.Temperature = int(c.Temperature)
			entity.State = climate
			p.store.Save(entity)
		}
	default:
